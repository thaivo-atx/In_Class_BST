...
void BinTree::addNodes(int ids[], string strings[]) {
     ...
     * It is for testing displayInOrder traversal and nothing else.
     */
    root = new DataNode;
    root->left = NULL;
    root->right = NULL;
...
void BinTree::displayInOrder(DataNode *temproot) {
    ...
    // this commented out code also works but recurses twice as much
    // simpler to read and understand, but probably will take a performance
    // hit it if the tree is very larg
    /* 
    if (temproot) {
    ...
    */   
    ...
    //this code is logically the same, but recurses half as much
    if (temproot) {
        if (temproot->left) {
            displayInOrder(temproot->left);
        }
